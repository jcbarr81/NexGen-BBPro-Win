    ...
    >
      ...
      {team.isLoading ? (
        ...
        <div className="space-y-6 animate-fade-in">
          <HeroCard team={team.data} snapshot={snapshot.data} />
          ...
          <section className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
            <StatCard
              ...
              Icon={TrendingUp}
              tone={
                parseRunDiff(snapshot.data?.run_diff) >= 0 ? "success" : "danger"
              }
            />
            <StatCard
              label="Streak"
            ...
            <StatCard
              label="Active Roster"
              value={roster.data?.active_size ?? "—"}
              sub={`${(roster.data?.levels.AAA?.length ?? 0) + (roster.data?.levels.LOW?.length ?? 0)} in minors`}
              Icon={UsersIcon}
            />
            <StatCard
              label="Next Game"
    ...
    >
      <div className="relative flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <div className="flex items-center gap-5">
          <div
            className="flex h-20 w-20 items-center justify-center rounded-2xl border border-border font-display text-3xl font-bold shadow-panel"
            style={{
              ...
              {rows.map((row) => (
                <tr
                  ...
                  className={cn(
                    ...
                    row.team_id === teamId && "bg-amber/10",
                  )}
                >
